  ...
  return models.question
    ...
    .then((questions) => {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        let secondsDiff = moment(current.exit_time).diff(
          moment(current.help_time),
          'seconds'
        );
      ...
      let currentlyHelping = home
        .getOHQ()
        .getAllStudentData()
        .filter((studentData) => studentData.helpTime !== null);
      for (let i = 0; i < currentlyHelping.length; i++) {
        let current = currentlyHelping[i];
        let secondsDiff = moment
          .tz(new Date(), 'America/New_York')
          .diff(moment(current.helpTime), 'seconds');
        totalHelpedSeconds += secondsDiff;
      ...
      if (totalQuestions != 0) {
        ...
        if (minute_ago_time == null) {
          ...
          if (
            last_pinged == null ||
            now.diff(last_pinged, 'seconds') >= ping_interval_mins * 60
          ) {
            if (
              waitTime > ping_threshold_mins &&
              minute_ago_waittime > ping_threshold_mins
            ) {
              slack.send_message(
                '<!channel> The wait time is ' +
                  Math.round(waitTime) +
                  ' minutes right now. More TAs might be needed. (<' +
                  config.PROTOCOL +
                  '://' +
                  config.DOMAIN +
                  '/|view»>)'
