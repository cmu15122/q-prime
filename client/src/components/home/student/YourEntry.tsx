...
export default function YourEntry(props) {
  ...
  const cooldownMsg = queueData.allowCDOverride ?
    ...
    <BaseCard>
      <CardContent sx={{ m: 1, textAlign: 'left' }}>
        ...
        <Typography variant="h6">
          ...
          <strong>
            {queueData.numTAs * studentData.position === 0 ?
              0 :
              Math.floor(
                  (queueData.minsPerStudent / queueData.numTAs) *
        ...
        <Typography variant="h6">
          <strong>Topic:</strong> {studentData.topic.name}
        </Typography>
        <CustomDivider />
